...
Collection.prototype.findOneAndDelete = function(query, options={}) {
  options.remove = true
  this.findAndModify(query, options);
}


Collection.prototype.findOneAndUpdate = function findOneAndUpdate(query, doc, options) {
  const {
    sort,
    upsert=false,
    projection={},
    fields=projection } = options

  this.findAndModify(filter, sort, doc, assign({
    new:true,
    update: true,
...
Collection.prototype.findAndModify = function(query, sort=[], doc, options) {
  ...
  const qo = { query, remove, upsert, serializeFunctions, 'new': !options.new, findandmodify: this.s.name }

  if (sort = formattedOrderClause(sort))
    qo.sort = sort;
  ...
  var finalOptions = writeConcern(options, this.s.db, this, options);

  // Decorate the  command with the write Concern
  if (finalOptions.writeConcern)
    qo.writeConcern = finalOptions.writeConcern;

  // Have we specified bypassDocumentValidation
  if(typeof finalOptions.bypassDocumentValidation == 'boolean')
    qo.bypassDocumentValidation = finalOptions.bypassDocumentValidation;

  // Have we specified collation
  decorateWithCollation(qo, this, options);

  return new Promise((done, fail) => this.s.db.command(qo, options, (err, res) => err ? fail(err) : done(res)))
}
...
var getReadPreference = function(self, opts, db) {
  var r = opts.readPreference||self.s.readPreference||db.s.readPreference;


  if (r instanceof ReadPreference)
    opts.readPreference = new CoreReadPreference(r.mode, r.tags, { maxStalenessSeconds: r.maxStalenessSeconds});

  else if(typeof r == 'string')
    opts.readPreference = new CoreReadPreference(r);

  else if(r && !(r instanceof ReadPreference) && typeof r == 'object') {
    var mode = r.mode || r.preference;
    if (mode && typeof mode == 'string')
      opts.readPreference = new CoreReadPreference(mode, r.tags, {maxStalenessSeconds: r.maxStalenessSeconds});
  }
